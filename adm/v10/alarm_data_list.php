...
$manager_display = (!$member['mb_manager_yn'] && auth_check($auth[$sub_menu],"w",1)!='') ? 'none' : '';   // manager가 아니면 display:none;
...
$mms_search = " WHERE ".$pre."_status NOT IN ('trash','delete') AND arm_cod_type IN ('a','p','p2') ";   // 설비 옵션 선택을 위한 검색조건
...
//                    $list[$k1] = substr($row[$k1],0,10);
                    ...
                    // $list[$k1] = $row[$k1].' <span class="font_size_8">'.$g5['category_up_names'][$row['trm_idx_category']].'</span>';
                }
                else if($k1=='arm_code') {
